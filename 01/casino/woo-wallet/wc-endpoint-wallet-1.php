<link rel="stylesheet" href="<?php echo get_template_directory_uri() ?>/assets/css/wallet.css">
<?php
/**
...
<div class="woo-wallet-my-wallet-container">
    <ul>
        <?php if (apply_filters('woo_wallet_is_enable_top_up', true)) : ?>
            <li class="card"><a href="<?php echo is_account_page() ? esc_url(wc_get_endpoint_url(get_option('woocommerce_woo_wallet_endpoint', 'woo-wallet'), 'add', wc_get_page_permalink('myaccount'))) : add_query_arg('wallet_action', 'add', get_permalink()); ?>"><span class="fas fa-dollar-sign"></span>
                    <p><?php echo apply_filters('woo_wallet_account_topup_menu_title', __('Deposit', 'woo-wallet')); ?></p>
                </a></li>
        <?php endif; ?>
        ...
        <?php if (apply_filters('woo_wallet_is_enable_transfer', 'on' === woo_wallet()->settings_api->get_option('is_enable_wallet_transfer', '_wallet_settings_general', 'on'))) : ?>
            <li class="card"><a href="<?php echo is_account_page() ? esc_url(wc_get_endpoint_url(get_option('woocommerce_woo_wallet_endpoint', 'woo-wallet'), 'transfer', wc_get_page_permalink('myaccount'))) : add_query_arg('wallet_action', 'transfer', get_permalink()); ?>"><span class="dashicons dashicons-randomize"></span>
                    <p><?php echo apply_filters('woo_wallet_account_transfer_amount_menu_title', __('Transfer Funds', 'woo-wallet')); ?></p>
                </a></li>
        <?php endif; ?>

        <?php if (apply_filters('woo_wallet_is_enable_transaction_details', true)) : ?>
            <li class="card"><a href="<?php echo home_url(); ?>/my-account/wallet/"><span class="dashicons dashicons-list-view"></span>
                    <p><?php echo apply_filters('woo_wallet_account_transaction_menu_title', __('Transactions', 'woo-wallet')); ?></p>
                </a></li>
        <?php endif; ?>
        <?php do_action('woo_wallet_menu_items'); ?>
        <li class="card"><a href="<?php echo home_url(); ?>/my-account/wallet-withdrawal/"><span class="dashicons dashicons-randomize"></span>
                <p><?php echo 'Withdraw Funds'; ?></p>
            </a></li>
    </ul>
    <div class="woo-wallet-content">
            ...
            <?php } else if (apply_filters('woo_wallet_is_enable_transfer', 'on' === woo_wallet()->settings_api->get_option('is_enable_wallet_transfer', '_wallet_settings_general', 'on')) && ((isset($wp->query_vars['woo-wallet']) && 'transfer' === $wp->query_vars['woo-wallet']) || (isset($_GET['wallet_action']) && 'transfer' === $_GET['wallet_action']))) { ?>
                <form method="post" action="">
                    <p class="woo-wallet-field-container form-row form-row-wide">
                        <label for="woo_wallet_transfer_user_id"><?php _e('Select whom to transfer', 'woo-wallet'); ?>
                            <?php if (apply_filters('woo_wallet_user_search_exact_match', true)) {
                                _e('(Email)', 'woo-wallet');
                            }     ?></label>
        ...
        <?php } else if (apply_filters('woo_wallet_is_enable_transaction_details', true)) { ?>
            ...
            $transactions = get_wallet_transactions(array('limit' => apply_filters('woo_wallet_transactions_count', $limit), 'pageno' => $pageno));
            $transactions_count = get_count_transactions_api();
            ...
            <?php if (!empty($transactions)) { ?>
                <ul class="woo-wallet-transactions-items">
                    <?php foreach ($transactions as $transaction) : ?>
                        <li>
                            <div>
                                <p> <?php echo $transaction->status ? $transaction->status : '-'; ?></p>
                                <small><?php echo wc_string_to_datetime($transaction->createdAt)->date_i18n('F j, Y  H:i:s'); ?></small>
                            </div>
                            <div class="woo-wallet-transaction-type-<?php echo $transaction->action; ?>">
                                <?php echo $transaction->action == 'credit' ? '+' : '-';
                                echo wc_price(apply_filters('woo_wallet_amount', $transaction->amount, $transaction->currency, $transaction->user_id)); ?></div>
                        </li>
                    <?php endforeach; ?>
                </ul>
                ...
                <ul class="table-paging">
                    <?php
                    $totalNoPage = ceil($transactions_count / $limit);
                    if ($totalNoPage) {
                        for ($i = 1; $i <= $totalNoPage; $i++) {
                    ?>
                            <li class=""> <a <?php if ($pageno == $i) {
                                                    echo 'class="is-active"';
                                                } ?> href="<?php echo get_permalink(752767); ?>?pageno=<?php echo $i; ?>"> <?php echo $i; ?></a></li>
                    <?php
                        }
                    }
                    // echo  "transactions_count :- " . $transactions_count;
                    ?>
                </ul>
        <?php
            } else {
